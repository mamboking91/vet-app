...
import type { BloquePagina, ContenidoHeroe, ContenidoCaracteristicas, ContenidoProductosDestacados, ContenidoTextoConImagen, ContenidoCTA, ContenidoInstagram } from '@/app/dashboard/configuracion/contenido/types';
import ProductImage from '@/components/ui/ProductImage';
import React from 'react';
import { cn } from '@/lib/utils';
import { ArrowRight, ShoppingBag, HeartPulse, Stethoscope, Siren, Sparkles, LucideProps, Instagram } from 'lucide-react';
...
function InstagramBlock({ contenido }: { contenido: ContenidoInstagram }) {
    if (!contenido.posts || contenido.posts.length === 0) return null;
  
    return (
      <section className="py-16 bg-white dark:bg-slate-800/50">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-bold text-center text-gray-800 dark:text-white mb-4">{contenido.titulo}</h2>
          <div className="flex justify-center mb-10">
              <Button asChild variant="outline">
                  <a href="https://www.instagram.com/gomera_mascotas/" target="_blank" rel="noopener noreferrer">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {contenido.posts.map(post => (
              <a href={post.enlace} key={post.id} target="_blank" rel="noopener noreferrer" className="group block aspect-square overflow-hidden rounded-lg shadow-md hover:shadow-xl transition-shadow">
                  <Image
                      src={post.imagenUrl || 'https://placehold.co/300x300/e2e8f0/e2e8f0.png?text=IG'}
...
export default async function HomePage() {
  ...
  const renderBlock = (bloque: BloquePagina) => {
    switch (bloque.tipo_bloque) {
      ...
      case 'instagram': return <InstagramBlock key={bloque.id} contenido={bloque.contenido} />;
